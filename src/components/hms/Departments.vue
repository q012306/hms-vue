<template>
  <div>
    <el-table
      :data="departments.filter(data => !search || data.ename.toLowerCase().includes(search.toLowerCase()))"
      style="width: 100%">
      <el-table-column
        prop="sid"
        label="ID 号">
      ...
      <el-table-column align="right">
        ...
        <template slot="header" slot-scope="scope">
          <el-input
            ...
            v-model="search"
            size="mini"
            placeholder="输入部门名关键字搜索"/>
        </template>
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="editDepartment(scope.row)">编辑</el-button>
          <el-button
            size="mini"
            type="danger"
            @click="deleteDepartment(scope.row.sid)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      deleteDepartment (sid) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          type: 'warning'
            ...
            this.axios
              .post('/departmentsdelete', {sid: sid}).then(resp => {
